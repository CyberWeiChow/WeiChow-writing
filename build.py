...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Bookshelf: tilted upright slabs with simulated thickness */
    .cd-shelf {
      display: flex;
      align-items: flex-end;
      padding: 100px 80px 64px 96px;
      perspective: 2000px;
      perspective-origin: 35% 75%;
      overflow-x: auto;
      overflow-y: visible;
    ...
    /* the shelf line under the books */
    .cd-shelf::after {
      content: '';
      position: absolute;
      left: 60px;
    ...
    .cd {
      flex: 0 0 auto;
      width: 132px;
      height: 196px;                       /* 2:3 book aspect */
      margin-right: -56px;                 /* moderate overlap — spines mostly visible */
      position: relative;
      cursor: pointer;
      transform: rotateY(-22deg) rotateX(0deg) translateZ(0);
      transform-origin: 70% bottom;
      transform-style: preserve-3d;
      transition: transform 420ms var(--ease-spring), z-index 0s 420ms;
    }
    .cd:last-child { margin-right: 0; }
    /* depth offset: subtle receding stagger */
    .cd:nth-child(odd)  { transform: rotateY(-22deg) translateZ(-4px); }
    .cd:nth-child(even) { transform: rotateY(-22deg) translateZ(0); }
    .cd.active {
      transform: rotateY(-6deg) translateZ(36px) translateY(-10px);
      z-index: 20;
      transition: transform 420ms var(--ease-spring), z-index 0s;
    }
    @media (hover: hover) and (pointer: fine) {
      .cd:hover, .cd:focus-visible {
        transform: rotateY(0deg) translateZ(110px) translateY(-26px) scale(1.04);
        z-index: 100;
        transition: transform 420ms var(--ease-spring), z-index 0s;
      }
    }
    .cd:active { transform: rotateY(0deg) translateZ(95px) translateY(-20px) scale(1.0); }
    ...
    .cd-cover {
      ...
      background-position: center;
      background-color: var(--surface);
      /* stacked right-edge shadows simulate book thickness;
         drop shadow under suggests the book is standing */
      box-shadow:
        inset -2px 0 0 rgba(255,255,255,0.06),
        2px 0 0 rgba(0,0,0,0.45),
        5px 0 0 rgba(0,0,0,0.32),
        8px 0 0 rgba(0,0,0,0.22),
        11px 0 0 rgba(0,0,0,0.14),
        14px 0 0 rgba(0,0,0,0.08),
        12px 22px 40px rgba(0,0,0,0.45),
        4px 4px 16px rgba(0,0,0,0.3);
    }
    /* lighting overlay: subtle gradient simulates light from upper-left */
    .cd-cover::after {
      content: '';
      position: absolute;
      inset: 0;
    ...
    .cd:hover .cd-cover::after, .cd.active .cd-cover::after { opacity: 0.4; }
    ...
    @media (max-width: 600px) {
      .cd-shelf { padding: 64px 16px 40px 48px; }
      .cd { width: 108px; height: 160px; margin-right: -44px; }
    }
