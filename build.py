...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Literary criticism becomes one shared reading desk / critical apparatus. */
    .criticism-archive {
      position: relative;
      min-height: 760px;
      ...
      color: #201d19;
      background:
        linear-gradient(90deg, transparent 0 68px, rgba(158,48,42,.22) 68px 69px, transparent 69px),
        repeating-linear-gradient(180deg, rgba(40,35,29,.055) 0 1px, transparent 1px 34px),
        #eee6d6;
      border-top: 2px solid var(--fd-ink);
      border-bottom: 2px solid var(--fd-ink);
      isolation: isolate;
    ...
    .criticism-archive::after {
      content: 'CRITICAL APPARATUS · 批评装置';
      position: absolute;
      right: 18px;
      top: 18px;
      color: rgba(32,29,25,.52);
      font-family: var(--fd-mono);
      font-size: 9px;
      letter-spacing: .12em;
      writing-mode: vertical-rl;
    ...
    .archive-entry {
      ...
      align-items: center;
      color: inherit;
      border-top: 1px solid rgba(32,29,25,.55);
      background: rgba(247,241,228,.52);
      transition: transform 240ms cubic-bezier(.16,1,.3,1), background 180ms ease, opacity 180ms ease;
      outline: none;
    }
    .archive-entry:last-child { border-bottom: 1px solid rgba(32,29,25,.55); }
    .archive-entry::before {
      ...
      display: grid;
      place-items: center;
      border: 1px solid #9e302a;
      border-radius: 50%;
      color: #8d2925;
      background: #eee6d6;
      font-family: var(--fd-mono);
      font-size: 9px;
      transition: color 180ms ease, background 180ms ease, transform 220ms cubic-bezier(.16,1,.3,1);
    }
    .archive-entry.is-active { background: rgba(255,250,239,.82); }
    .archive-entry.is-active::before { color: #eee6d6; background: #9e302a; transform: scale(1.08); }
    .archive-entry:focus-visible { outline: 2px solid #9e302a; outline-offset: 4px; }
    .archive-entry:active { transform: scale(.985); }
    @media (hover: hover) and (pointer: fine) {
      .archive-entry:hover { transform: translateX(8px); background: rgba(255,250,239,.9); }
      .archive-ledger:has(.archive-entry:hover) .archive-entry:not(:hover) { opacity: .58; }
    }
    .archive-entry-copy small {
      display: block;
      margin-bottom: 10px;
    ...
    .archive-motif { position: relative; width: 142px; height: 108px; }
    .archive-motif.history .frame {
      position: absolute;
      width: 72px;
      height: 88px;
      border: 1px solid #29251f;
      background: rgba(62,57,48,.13);
    }
    .archive-motif.history .frame:first-child { left: 12px; top: 3px; transform: rotate(-5deg); }
    .archive-motif.history .frame:nth-child(2) { left: 52px; top: 15px; transform: rotate(5deg); background: rgba(158,48,42,.12); }
    .archive-motif.history .crack { position: absolute; left: 69px; top: -1px; width: 1px; height: 110px; background: #9e302a; transform: rotate(14deg); }
    .archive-motif.translation { display: grid; align-content: center; gap: 9px; }
    .archive-motif.translation i { display: block; height: 7px; background: #2e2a24; }
    .archive-motif.translation i:nth-child(1) { width: 88%; }
    .archive-motif.translation i:nth-child(2) { width: 68%; }
    .archive-motif.translation i:nth-child(3) { width: 96%; background: linear-gradient(90deg,#2e2a24 0 30%,#9e302a 30% 72%,#2e2a24 72%); }
    .archive-motif.translation i:nth-child(4) { width: 52%; }
    .archive-motif.translation::after { content: '译 / TRANS.'; position: absolute; right: 2px; bottom: 5px; padding: 4px 7px; color: #eee6d6; background: #34483d; font: 8px var(--fd-mono); letter-spacing: .08em; transform: rotate(-3deg); }
    .archive-motif.ornament .swatch { position: absolute; width: 46px; height: 76px; top: 14px; border-radius: 50% 50% 8px 8px; opacity: .9; }
    .archive-motif.ornament .swatch:nth-child(1) { left: 4px; background: #9e302a; transform: rotate(-8deg); }
    .archive-motif.ornament .swatch:nth-child(2) { left: 47px; background: #a78a51; transform: translateY(-7px); }
    .archive-motif.ornament .swatch:nth-child(3) { left: 90px; background: #42584a; transform: rotate(8deg); }
    .archive-motif.ornament::after { content: ''; position: absolute; left: 1px; right: 1px; top: 57px; height: 28px; border-top: 1px solid #eee6d6; border-radius: 50%; transform: rotate(-8deg); }
    .archive-lens {
      position: sticky;
      top: 34px;
      align-self: start;
      min-height: 520px;
      padding: 62px 0 28px 38px;
      border-left: 1px solid rgba(32,29,25,.48);
    }
    .archive-lens::before {
      content: '';
      position: absolute;
      ...
      right: -10%;
      top: 4px;
      border: 1px solid rgba(158,48,42,.5);
      border-radius: 50%;
      background: rgba(255,250,239,.28);
      box-shadow: inset 0 0 60px rgba(255,255,255,.36);
      pointer-events: none;
    }
    .archive-lens-content { position: relative; z-index: 1; transition: opacity 150ms ease, transform 220ms cubic-bezier(.16,1,.3,1), filter 150ms ease; }
    .archive-lens-content.is-changing { opacity: .35; transform: translateY(5px); filter: blur(1.5px); }
    .archive-lens-kicker { font-family: var(--fd-mono); font-size: 9px; letter-spacing: .13em; text-transform: uppercase; }
    .archive-lens h3 { margin: 76px 0 18px; max-width: 410px; font-family: 'Noto Serif SC', serif; font-size: clamp(24px, 3vw, 42px); line-height: 1.35; font-weight: 500; }
    .archive-keywords { margin: 0 0 22px; color: #8d2925; font-family: var(--fd-mono); font-size: 9px; letter-spacing: .12em; text-transform: uppercase; }
    .archive-abstract { max-width: 430px; margin: 0; display: -webkit-box; -webkit-line-clamp: 5; -webkit-box-orient: vertical; overflow: hidden; font-family: 'Noto Serif SC', serif; font-size: 12px; line-height: 1.9; }
    .archive-read { display: inline-flex; margin-top: 26px; padding-bottom: 3px; border-bottom: 1px solid currentColor; font-family: var(--fd-mono); font-size: 9px; letter-spacing: .1em; text-transform: uppercase; }
    @media (max-width: 800px) {
      .criticism-archive { grid-template-columns: 1fr; min-height: auto; padding: 64px 18px 70px; gap: 48px; }
      .archive-ledger { padding-left: 48px; }
      .archive-entry { grid-template-columns: 112px 1fr; gap: 16px; padding-left: 12px; }
      .archive-motif { width: 112px; transform: scale(.82); transform-origin: left center; }
      .archive-entry::before { left: -48px; }
      .archive-lens { position: relative; top: auto; min-height: 420px; padding: 42px 0 0 24px; }
      .archive-lens::before { width: 250px; right: 0; }
    }
    @media (max-width: 520px) {
      .archive-entry { min-height: 132px; grid-template-columns: 78px 1fr; }
      .archive-motif { width: 96px; transform: scale(.66); }
      .archive-entry-copy strong { font-size: 15px; }
    ...
    function showCurated(initialSection) {
      ...
      function criticismArchive() {
        ...
        const motifHTML = (type) => type === 'history'
          ...
          : type === 'translation'
            ? '<div class="archive-motif translation"><i></i><i></i><i></i><i></i></div>'
            : '<div class="archive-motif ornament"><i class="swatch"></i><i class="swatch"></i><i class="swatch"></i></div>';
        ...
        return `<div class="criticism-archive" id="criticism-archive">
          ...
          <aside class="archive-lens">
            <div class="archive-lens-content" id="archive-lens-content">
              <div class="archive-lens-kicker">Reading dossier · ${works.length} studies</div>
              <h3 id="archive-lens-title">${escapeHtml(first.title || '文学批评')}</h3>
              <p class="archive-keywords" id="archive-lens-keywords">${escapeHtml(firstData.keywords)}</p>
              <p class="archive-abstract" id="archive-lens-abstract">${escapeHtml(first.subtitle || '从文本进入历史的裂缝。')}</p>
              ${first.id ? `<a class="archive-read" id="archive-lens-link" href="#/read/${encodeURIComponent(first.id)}">open dossier ↗</a>` : ''}
            </div>
          </aside>
        </div>`;
      }
      ...
      const criticismDesk = document.getElementById('criticism-archive');
      if (criticismDesk) {
        const criticismWorks = (MANIFEST.works || []).filter(w => w.category === 'criticism');
        const lensContent = document.getElementById('archive-lens-content');
        const lensTitle = document.getElementById('archive-lens-title');
        const lensKeywords = document.getElementById('archive-lens-keywords');
        const lensAbstract = document.getElementById('archive-lens-abstract');
        const lensLink = document.getElementById('archive-lens-link');
        let lensTimer = null;
        const activateArchiveEntry = (entry) => {
          const work = criticismWorks.find(w => w.id === entry.dataset.id);
          if (!work) return;
          criticismDesk.querySelectorAll('.archive-entry').forEach(el => el.classList.toggle('is-active', el === entry));
          lensContent.classList.add('is-changing');
          clearTimeout(lensTimer);
          lensTimer = setTimeout(() => {
            lensTitle.textContent = work.title;
            lensKeywords.textContent = entry.dataset.keywords || 'reading · history · text';
            lensAbstract.textContent = work.subtitle || '进入文本';
            if (lensLink) lensLink.href = '#/read/' + encodeURIComponent(work.id);
            lensContent.classList.remove('is-changing');
          }, 110);
        };
        criticismDesk.querySelectorAll('.archive-entry').forEach(entry => {
          entry.addEventListener('pointerenter', () => activateArchiveEntry(entry));
          entry.addEventListener('focus', () => activateArchiveEntry(entry));
        });
        onViewCleanup(() => clearTimeout(lensTimer));
      }
